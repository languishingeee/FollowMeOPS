...
const app = {
    ...
    ui: {
        ...
        showPerformanceReport: (filter = 'today') => {
            ...
            // Personel başına tamamlanan uçuş sayıları
            const counts = {};
            ...
            app.state.completed.forEach(fid => {
                ...
                if (staffName && counts.hasOwnProperty(staffName)) {
                    // Uçuş timestamp'ini kontrol et
                    const flight = app.state.flights.find(f => f.id === fid);
                    if (flight) {
                        ...
                        // Uçuş bulunamazsa yine de say (eski veriler)
                        if (filter === 'all') counts[staffName]++;
                    }
                }
            });

            // En yüksek değeri bul (bar chart için)
            ...
            // Toplam
            const filterLabel = filter === 'today' ? 'Bugün' : filter === 'week' ? 'Bu Hafta' : filter === 'month' ? 'Bu Ay' : 'Toplam';
            total.innerHTML = `
                ...
                <div class="text-xs text-gray-500 mt-1">${app.state.staff.length} personel</div>
            `;
